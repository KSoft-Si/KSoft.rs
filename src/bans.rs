...
impl Bans {
    ...
    pub async fn advanced_paginate(&self, page: u8, per_page: u8) -> HttpResult<BanList, Error400>{
    ...
    pub async fn paginate(&self) -> HttpResult<BanList, Error400> {
    ...
    -> HttpResult<BanAdditionResponse, Error409>{
    ...
    pub async fn ban_info(&self, user_id: u64) -> HttpResult<BanInfoResponse, Error404> {
    ...
    pub async fn delete_forcing(&self, user_id: u64) -> HttpResult<BanDeletionResponse, Error400> {
        let builder = self.http.clone().delete(endpoint("/bans/delete").as_str())
            .query(&[("user", user_id)])
            .query(&[("force", true)]);

        make_request::<BanDeletionResponse, Error400>(builder).await
    }

    pub async fn delete(&self, user_id: u64) -> HttpResult<BanDeletionResponse, Error400> {
        let builder = self.http.clone().delete(endpoint("/bans/delete").as_str())
            .query(&[("user", user_id)]);
